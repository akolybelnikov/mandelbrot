...
/// precisely, if we have reached the iteration limit without being able to prove that `c` is not a
/// member), return `None`.
...
/// Convert from image space to complex number space: each pixel int he output image corresponds to
...
fn pixel_to_point(
    ...
    Complex {
        ...
        // pixel.1 increases as we go down, but the imaginary component increases as we go up, hence
        // subtraction here
